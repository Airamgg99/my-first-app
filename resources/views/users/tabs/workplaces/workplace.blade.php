<div class="flex flex-col h-full">
    <div class="w-full">
        <div class="flex flex-col sm:flex-row justify-between space-y-3 sm:space-y-0 sm:space-x-3 pb-3">
            <div class="flex space-x-3 items-center justify-start">
                <x-buttons.toggleTable toggle="toggleUnlinkedWorkplaces" :showUnlinked="$showUnlinkedWorkplace" />
            </div>
        </div>
        @if (!$showUnlinkedWorkplace)
            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pb-3">
                <x-inputs.search search="Workplaces search" />
            </div>
                        ...
                        @foreach ($workplaces as $workplace)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 flex justify-center">
                                    <x-buttons.unlink :id="$workplace->id" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                    ...
                    <thead class="space-y-4">
                        @foreach ($workplaces as $workplace)
                            <tr class="bg-[#1B3D73] text-[#FDC700] flex flex-col wrap sm:table-row">
                                ...
                                <th class="h-16 p-3 text-center content-center">Address</th>
        ...
        @else
            ...
            <div class="w-full">
                @livewire('users.show.workplaces.unlinked-workplaces', ['user' => $user])
            </div>
        @endif
