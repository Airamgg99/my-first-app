<div class="w-full">
    <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pb-3">
        <x-inputs.search search="Unlinked contract types search" />
    </div>
    ...
    <div class="hidden sm:block">
        <table class="w-full text-sm text-left">
            ...
            <tbody>
                @foreach ($unlinkedContractType as $contractType)
                    <tr class="bg-white border-b hover:bg-gray-50">
                        <td class="px-6 py-4 text-center">
                            {{ $contractType->name }}
                        </td>
                        <td class="px-6 py-4 flex justify-center">
                            <x-buttons.link :id="$contractType->id" />
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="block sm:hidden">
        <table class="w-full flex wrap rounded-lg md:hidden">
            <thead class="space-y-4">
                @foreach ($unlinkedContractType as $contract_type)
