...
@section('content')
                ...
                <div class="relative bg-blue-100 rounded-lg shadow">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="modal-body grid grid-cols-6 gap-6">
                            ...
                            <div class="form-group col-span-6 sm:col-span-3">
                                <label for="user" class="text-lg font-medium text-gray-900 block mb-2">Users</label>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <label for="start" class="text-lg font-medium text-gray-900 block">Start
                                    Date</label>
                                <input type='date' class='form-control' id='startDate' name='startDate' required>
                                <input type='time' class='form-control mt-2' id='start' name='start' required>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label for="end" class="text-lg font-medium text-gray-900 block">End Date</label>
                ...
                <div class="relative bg-blue-100 rounded-lg shadow">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="modal-body grid grid-cols-6 gap-6">
                            ...
                            <div class="form-group col-span-6 sm:col-span-3">
                                <label for="user" class="text-lg font-medium text-gray-900 block mb-2">Users</label>
                                <select id="edit-user" name="user_id" required
                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm focus:ring-cyan-600 focus:border-cyan-600 block w-full p-3">
                                    <option value="" selected disabled>Select a user</option>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}">{{ $user->name }}</option>
                            ...
                            <div class="form-group col-span-6 sm:col-span-3">
                                ...
                                <select id="edit-workplaces" name="workplace_id" required
                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm focus:ring-cyan-600 focus:border-cyan-600 block w-full p-3">
                                    <option value="" selected>Select a workplace</option>
                                </select>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label for="edit-startDate" class="text-lg font-medium text-gray-900 block">Start
                                    Date</label>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <label for="edit-endDate" class="text-lg font-medium text-gray-900 block">End
                                    Date</label>
    ...
    {{-- * Calendario * --}}
    <div id="calendar" data-admin-id="{{ Auth::id() }}" class="px-3 pt-10 w-full">
    </div>
    ...
    <script>
        ...
        // Para evitar duplicados en los selects
        $('#user').off('change');
        $('#edit-user').off('change');
        ...
        var id = calendarEl.getAttribute('data-admin-id');
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            select: function(info) {
                ...
                // Actualizar centros de trabajo cuando se cambia de usuario
                $('#user').on('change', function() {
                    var userId = $(this).val();
                    var $workplaceSelect = $('#workplaces');
                    $workplaceSelect.empty().append(
                        '<option value="">Select a workplace</option>'); // Resetear opciones

                    if (userId) {
                        $.ajax({
                            url: '/admin/get-workplaces/' + userId,
                            method: 'GET',
                            success: function(response) {
                                console.log('Received workplaces:',
                                    response);
                                response.forEach(function(workplace) {
                                    console.log('Adding workplace:',
                                        workplace);
                                    $workplaceSelect.append('<option value="' +
                            ...
                            error: function() {
                                Swal.fire({
                                    title: "Error",
                                    text: "Could not load workplaces.",
                                    icon: "error",
                ...
                // Actualizar centros de trabajo cuando se cambia de usuario en el edit
                $('#edit-user').on('change', function() {
                    var userId = $(this).val();
                    var $workplaceSelect = $('#edit-workplaces');
                    $workplaceSelect.empty().append('<option value="">Select a workplace</option>');
                    ...
                    if (userId) {
                        $.ajax({
                            ...
                            method: 'GET',
                            success: function(response) {
                                console.log('Received workplaces for edit:',
                                    response);
                                response.forEach(function(workplace) {
                                    console.log('Adding workplace for edit:',
                                        workplace);
                                    $workplaceSelect.append('<option value="' +
                                        workplace.id + '">' + workplace.name +
                                        '</option>');
            ...
            eventClick: function(info) {
                ...
                // Se coloca el usuario y sus centros de trabajo
                $('#edit-user').val(info.event.extendedProps.user_id).trigger('change');
                var userId = info.event.extendedProps.user_id;
                var selectedWorkplaceId = info.event.extendedProps.workplace_id;
                var $workplaceSelect = $('#edit-workplaces');

                // Trae los centros de trabajo relacionados con el usuario determinado
                if (userId) {
                    $.ajax({
                        url: '/admin/get-workplaces/' + userId,
                        method: 'GET',
                        success: function(response) {
                            console.log('Received workplaces for edit modal:',
                                response);
                            $workplaceSelect.empty().append(
                                '<option value="">Select a workplace</option>');
                            response.forEach(function(workplace) {
                                console.log('Adding workplace for edit modal:',
                                    workplace);
                                $workplaceSelect.append('<option value="' + workplace.id +
                                    '">' + workplace.name + '</option>');
                            });
                            $workplaceSelect.val(selectedWorkplaceId);
                        },
                        error: function() {
                            Swal.fire({
                                title: "Error",
                ...
                // Actualizar centros de trabajo cada vez que se cambie de usuario
                $('#edit-user').on('change', function() {
                    var userId = $(this).val();
                    $workplaceSelect.empty().append('<option value="">Select a workplace</option>');
                    ...
                    if (userId) {
                        $.ajax({
                            ...
                            method: 'GET',
                            success: function(response) {
                                console.log('Received workplaces on edit user change:',
                                    response);
                                response.forEach(function(workplace) {
                                    console.log(
                                        'Adding workplace on edit user change:',
                                        workplace);
                                    $workplaceSelect.append('<option value="' +
                                        workplace.id + '">' + workplace.name +
                                        '</option>');
